/// ENLOpticalSimTrackingAction.cc
...
ENLOpticalSimTrackingAction::ENLOpticalSimTrackingAction(){}
ENLOpticalSimTrackingAction::~ENLOpticalSimTrackingAction(){}
void ENLOpticalSimTrackingAction::PreUserTrackingAction(const G4Track* aTrack){
  ...
  if( partname == "opticalphoton" && aTrack->GetUserInformation() == 0 ){
	  ...
	  theTrack->SetUserInformation(new ENLOpticalSimTrackInformation(aTrack));
      }
...
void ENLOpticalSimTrackingAction::PostUserTrackingAction(const G4Track* aTrack){
  ...
  if(partname == "opticalphoton"){
	  ...
	  if(nSeco > 0){
		  ...
		  if(cproc == "OpWLS") {
		      (*secondaries)[i]->SetUserInformation(new ENLOpticalSimTrackInformation(aTrack));
		  }
	      }
	  }
      }
